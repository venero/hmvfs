...
int mmap_file(char* filename) {
    ...
    char *mapped_area;
    if ((mapped_area = mmap(NULL, BLK_SIZE, PROT_READ | PROT_WRITE, MMAP_MODE, fd, 0)) == MAP_FAILED) {
        perror("mmap");
        close(fd);
        return 1;
    }
    ...
    if ((mapped_area = mmap(NULL, BLK_SIZE, PROT_READ | PROT_WRITE, MMAP_MODE, fd, 2 * BLK_SIZE)) == MAP_FAILED) {
        perror("mmap");
        close(fd);
        return 1;
    }
    ...
    if ((mapped_area = mmap(NULL, BLK_SIZE, PROT_READ | PROT_WRITE, MMAP_MODE, fd, BLK_SIZE)) == MAP_FAILED) {
        perror("mmap");
        close(fd);
        return 1;
    }
    fill_buf(mapped_area, BLK_SIZE, 'c');
    return 0;
}
