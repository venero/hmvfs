...
static inline u64 free_user_blocks(struct hmfs_sb_info *sbi)
{
	if (free_segments(sbi) < overprovision_segments(sbi))
		...
		return (free_segments(sbi) -
			overprovision_segments(sbi)) << HMFS_PAGE_PER_SEG_BITS;
}
...
static inline bool has_enough_invalid_blocks(struct hmfs_sb_info *sbi)
{
	...
	unsigned long invalid_user_blocks =
	    cm_i->alloc_block_count - cm_i->valid_block_count;
