...
static int prepare_move_arguments(struct gc_move_arg *arg,
		struct hmfs_sb_info *sbi, unsigned segno, unsigned offset,
		...
		int src_off, struct hmfs_summary *src_sum)
{
	...
	while (args.start_version != args.dead_version) {
		...
		BUG_ON(le64_to_cpu(this->dn.addr[args.ofs_in_node]) != args.src_addr);
...
static void move_node_block(struct hmfs_sb_info *sbi, unsigned long src_segno,
		unsigned int src_off, struct hmfs_summary *src_sum, int type)
{
	...
	while (args.start_version != args.dead_version) {
		...
		BUG_ON(
				le64_to_cpu(this->entries[args.ofs_in_node].block_addr)
						!= args.src_addr);

		this->entries[args.ofs_in_node].block_addr = cpu_to_le64(args.src_addr);
		last = this;
		...
		struct hmfs_summary *src_sum, int type)
{
	...
	while (args.start_version != args.dead_version) {
		...
		if (++args.nrchange >= args.count)
			break;
		next: if (!args.dead_version && args.cp_i == cm_i->last_cp_i)
			break;
...
int hmfs_gc(struct hmfs_sb_info *sbi, int gc_type)
{
	...
	gc_more: if (!(sbi->sb->s_flags & MS_ACTIVE))
		goto out;
