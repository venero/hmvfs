...
for ((i=0; i<$num_file; i++)); do
    ...
    ln -s ${WORK_DIR}test_file_$i.txt ${WORK_DIR}test_file_${i}_slink.txt
done

echo "$num_file soft links created..."
...
for ((i=0; i<$num_file; i++)); do
    rm ${WORK_DIR}test_file_${i}_slink.txt
done

echo "$num_file soft links deleted..."
